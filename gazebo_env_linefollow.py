...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        middleImage = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)
        _,bwImage = cv2.threshold(middleImage, 127, 255, cv2.THRESH_BINARY)
        
        #convert it into one array, extract black indices, find median
        visionLine = bwImage[-50,:]
        blacki = np.where(visionLine==0)[0]

        #if line was found
        if len(blacki) > 0:
            center = np.median(blacki)

            #determine our state
            xLength = len(visionLine)
            stateLength = float(xLength/10.0)
            stateIndex = max(0, min(9, int(center / stateLength)))
